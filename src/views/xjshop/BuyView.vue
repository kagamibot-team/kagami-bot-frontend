...
const cost = computed(() => formatter.format(
  data.value.records.map(d => d.price).reduce((a, b) => a + b)
));

const qrcode_data = computed(() => JSON.stringify(data));
</script>
...
<template>
  <div class="main">
    ...
    <div v-for="(item, index) of data.records" :key="index">
      -&ensp;{{ item.title }}
      <div class="right">
        {{ formatter.format(item.price) }}&ensp;薯片
      </div>
      <div>&ensp;</div>
    </div>
    <hr>
    ...
    <div>
      余额
      <div class="right">
        {{ formatter.format(data.remain_chips) }} 薯片
      </div>
    </div>
    <div class="qrcode">
