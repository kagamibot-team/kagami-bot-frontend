...
export function rep_str_to_ids(str: string): Array<string> {
    ...
    for (let pointer = 0; pointer < str.length; pointer++) {
        ...
        if (met_symbol) {
            ...
            if (lpointer == -1) {
                ...
                // 找对应的字形 SVG 文件。
                result.push(query_ids_to_code(str.substring(lpointer, pointer + 1)));
...
function query_ids_to_code(ids: string): string {
    let ids_array = [];
    for (let index = 0; index < ids.length; index++) {
        ids_array.push('u' + ids.charCodeAt(index).toString(16));
    }
