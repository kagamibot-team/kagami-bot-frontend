...
const notations_down = computed(() => {
  if (props.now_merging) {
    return [
      '→' + props.info.after_storages[0],
      '→' + props.info.after_storages[1],
      '→' + props.info.after_storages[2],
      '+' + props.info.output.count
    ]
  }
  let ret = ['', '', '', '']
  ...
  <div class="outbox" :class="{ 'outbox-opacity': props.is_opacity }">
    <div class="recipe">
      <div class="recipe-material">
        <BookBoxSmall
          v-for="(item, index) in info.inputs"
          :key="index"
          :display_box="{
...
.outbox-opacity {
  background: linear-gradient(to bottom, #e8dcc96f, rgba(70, 63, 54, 0.518));
